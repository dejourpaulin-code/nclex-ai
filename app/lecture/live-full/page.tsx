...
  }
  ...
  async function generateAndDownloadStudyGuide() {
    if (!sessionId || !canUseLiveFull) return;
    ...
    try {
      ...
      // Build HTML for the PDF-ready study guide
      const d = data as {
        lectureTitle?: string;
...
${(d.quickReferenceNotes || []).map(n => `<div class="ref-note">&#8226; ${n}</div>`).join("")}
      ...
      // Open print dialog with the HTML — user can Save as PDF from there
            ...
            <div className="rounded-2xl border border-orange-100 bg-white p-4 shadow-sm">
              ...
              <div className="space-y-2">
                ...
                <button
                  ...
                  disabled={studyGuideLoading || !sessionId}
                  className="w-full rounded-xl border border-blue-300 bg-blue-50 py-2 text-sm font-semibold text-blue-800 transition hover:bg-blue-100 disabled:opacity-50"
                >
                  {studyGuideLoading ? "Generating..." : "Download Study Guide PDF"}
                </button>
                {studyGuideError && (
                  <p className="text-xs text-red-600">{studyGuideError}</p>
                )}
              </div>
            </div>
          </aside>
